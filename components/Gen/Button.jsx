...
const MagicButton = ({ title, icon, position, handleClick, otherClasses }) => {
  return (
    ...
    >
      ...
      <span className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-md bg-slate-950 px-6 py-1 text-sm font-medium gap-2 text-white backdrop-blur-3xl ">
        {position === "left" && icon}
        {title}
        {position === "right" && icon}
      </span>
